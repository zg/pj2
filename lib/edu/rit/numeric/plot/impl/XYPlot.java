...
// Unit:    Class edu.rit.numeric.plot.impl.XYPlot
//
...
// Hidden data members.
	 ...
	 * List of plot series.
	 */
	protected AList<PlotSeries> myPlotSeries = new AList<PlotSeries>();

	/**
	 * List of labels.
	 */
	protected AList<Label> myLabels = new AList<Label>();
...
// Exported operations.
	...
	public void addPlotSeries
		...
		myPlotSeries.addLast (thePlotSeries);
		}
	...
	public void draw
		...
		int len = myPlotSeries.size();
		for (int j = 0; j < len; ++ j)
			{
			PlotSeries thePlotSeries = myPlotSeries.get (j);
			...
			if (theStroke != null && thePaint != null)
				{
				// If there are no lines, draw nothing.
			...
			if (theDots != null)
				{
				int n = thePlotSeries.getDotCount();
				for (int i = 0; i < n; ++ i)
					{
					// Get point location.
