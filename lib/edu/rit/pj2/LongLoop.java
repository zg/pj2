...
// File:    LongLoop.java
// Package: edu.rit.pj2
// Unit:    Class edu.rit.pj2.LongLoop
//
...
/**
 * Class LongLoop is the abstract base class for a loop body executed by a
 * parallel for loop with a long integer index.
 * <P>
 * To execute a parallel for loop with a long integer index in the {@link
 ...
 * These are both of type <TT>long</TT>.
 * <P>
 * <B>Parallel thread team.</B>
 * The parallel for loop is executed by a team of threads. Each thread operates
 * on its own separate copy of the given LongLoop object. Each thread
 * automatically creates its own copy by calling the {@link #clone() clone()}
 * method on the given LongLoop object. Thus, any fields declared in the inner
 * LongLoop subclass are thread-local variables, not shared variables. However,
 * <I>such fields must not be initialized as part of their declarations.</I>
 * Instead, such fields must be initialized in the {@link #start() start()}
 * method. If fields of the inner LongLoop subclass are initialized as part of
 * their declarations, the cloning will not work properly.
 * <P>
 ...
 * Fields of the inner LongLoop subclass must be initialized in the {@link
 * LoopBody#start() start()} method. If the {@link LoopBody#start() start()}
 * method is not declared, the default is to do nothing.
 * <P><LI>
 * Call the {@link #run(long) run()} method zero or more times. On each call, a
 * different loop index is passed in. The {@link #run(long) run()} method must
 ...
 * Each thread calls the {@link #run(long) run()} method for the index or
 * indexes in the chunk or chunks assigned to that thread. See enum {@linkplain
 ...
 * {@link #run(long) run()} method. This causes <I>all</I> the threads in the
 * team to stop calling the {@link #run(long) run()} method (as soon as each
 ...
 * barrier, the {@link LongParallelForLoop#exec(LongLoop) exec()} method
 * returns.
 * <P>
 * <B>Reduction.</B>
 ...
 * public class MyTask extends Task
 *     {
 *     LongVbl counter = new LongVbl.Sum();
 *
 *     public void main (String[] args)
 *         {
 *         parallelFor (<I>lb, ub</I>) .exec (new LongLoop()
 ...
 * {@link edu.rit.pj2.vbl.LongVbl.Sum LongVbl.Sum}. The <TT>counter</TT>'s
 * {@link edu.rit.pj2.vbl.LongVbl#item item} field is an integer. The counter's
 * {@link edu.rit.pj2.vbl.LongVbl.Sum#reduce(Vbl) reduce()} method adds the
 * <TT>item</TT> fields together.
 * <P>
 * <TT>thrCounter</TT> is declared as a field of the inner LongLoop subclass,
 ...
 * <P><LI>
 * Execute the {@link #run(long) run()} method multiple times. Increment the
 * thread's own <TT>thrCounter</TT>'s {@link edu.rit.pj2.vbl.LongVbl#item item}
 ...
 * reduction is performed by the {@link edu.rit.pj2.vbl.LongVbl.Sum#reduce(Vbl)
 * reduce()} method, so all the threads' local counters' {@link
 * edu.rit.pj2.vbl.LongVbl#item item} fields are added together. The result is
 * stored back in the global variable's {@link edu.rit.pj2.vbl.LongVbl#item
 * item} field, which is then printed.
 *
 ...
 * @version 19-Nov-2013
 */
public abstract class LongLoop
	extends LoopBody
	{
...
// Exported operations.
	...
	/**
	 * Execute the loop body for the given long integer index.
	 * <P>
