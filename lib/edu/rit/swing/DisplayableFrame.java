...
// Unit:    Class edu.rit.swing.DisplayableFrame
//
 ...
 * the window. A "PNG Setup" menu item allows specifying the scale factor and
 * pixel resolution when saving the display in a PNG file; the defaults are
 * scale factor = 100% and resolution = 300&times;300 pixels per inch.
 * <P>
...
// Hidden data members.
	...
	private int myPngScaleFactor = 100;
	private int myPngResolution = 300;
...
// Exported constructors.
	...
	private void initialize
		...
		JMenuItem pngSetupItem =
			new JMenuItem ("PNG Setup...", KeyEvent.VK_U);
		pngSetupItem.setAccelerator
			(KeyStroke.getKeyStroke (KeyEvent.VK_U, InputEvent.CTRL_MASK));
		pngSetupItem.addActionListener
			(new ActionListener()
				{
...
// Hidden operations.
	 ...
	 * "PNG Setup..." menu item.
	 */
	private void doPngSetup()
		{
		Rectangle2D bounds = myDisplayable.getBoundingBox();
		int w = (int) (bounds.getWidth() + 0.5);
		int h = (int) (bounds.getHeight() + 0.5);
		PngSetupDialog dialog = new PngSetupDialog
			(this, w, h, myPngScaleFactor, myPngResolution);
		dialog.setLocationRelativeTo (this);
		dialog.setVisible (true);
		if (dialog.isOkay())
			{
			myPngScaleFactor = dialog.getScaleFactor();
			myPngResolution = dialog.getResolution();
			}
		dialog.dispose();
		}
			 ...
			 new FileSaver()
				...
				public void saveFile (File file) throws IOException
					...
					DisplayableIO.writeColorPNGFile
						(myDisplayable, file, myPngScaleFactor/100.0,
						 myPngResolution, myPngResolution);
					}
				});
		}
