...
// Unit:    Class edu.rit.swing.DisplayableIO
//
...
import edu.rit.image.BilevelPngWriter;
...
// Exported operations.
	 ...
	 * pixel) is used. The default physical pixel dimensions (300&times;300
	 * pixels per inch) are used.
	 *
	...
	public static void writeBilevelPNGFile
		...
		throws IOException
		{
		writeBilevelPNGFile (theDisplayable, theFile, 1.0);
		}

	/**
	 * Write the given displayable object to a PNG file in the form of a bilevel
	 * (black and white) image with the given scale factor. One display unit =
	 * <TT>theScale</TT> pixels. The default physical pixel dimensions
	 * (300&times;300 pixels per inch) are used.
	 *
	 * @param  theDisplayable  Displayable object to write.
	 * @param  theFile         Image file.
	...
	public static void writeBilevelPNGFile
		...
		throws IOException
		{
		writeBilevelPNGFile (theDisplayable, theFile, theScale, 300, 300);
		}

	/**
	 * Write the given displayable object to a PNG file in the form of a bilevel
	 * (black and white) image with the given scale factor and physical pixel
	 * dimensions. One display unit = <TT>theScale</TT> pixels.
	 *
	 * @param  theDisplayable  Displayable object to write.
	 * @param  theFile         Image file.
	 * @param  theScale        Scale factor for converting display units to
	 *                         pixels.
	 * @param  xPpi            Pixels per inch, X axis.
	 * @param  yPpi            Pixels per inch, Y axis.
	 *
	 * @exception  IOException
	 *     Thrown if an I/O error occurred.
	 */
	public static void writeBilevelPNGFile
		...
		final int w = (int) (bounds.getWidth() * theScale + 0.5);
		final int h = (int) (bounds.getHeight() * theScale + 0.5);
		...
		BilevelPngWriter writer = new BilevelPngWriter (h, w, out);
		writer.setPixelDimensions (xPpi, yPpi, PixelUnit.PIXELS_PER_INCH);
		final ByteImageQueue queue = writer.getImageQueue();
		new Thread()
			{
			public void run()
				{
				byte[] data = ((DataBufferByte) bi.getRaster().getDataBuffer())
					.getData();
				int len = (w + 7)/8;
				byte[] byteArray = new byte [w];
				for (int row = 0; row < h; ++ row)
					{
					for (int col = 0; col < w; ++ col)
						byteArray[col] = (byte)
							((data[(len*row)+(col>>3)] >> (7 - (col & 7))) & 1);
					try { queue.put (row, byteArray); }
	 ...
	 * is used. The default physical pixel dimensions (300&times;300 pixels per
	 * inch) are used.
	 *
	 * @param  theDisplayable  Displayable object to write.
	 * @param  theFile         Image file.
	...
	public static void writeGrayscalePNGFile
		...
		throws IOException
		{
		writeGrayscalePNGFile (theDisplayable, theFile, 1.0);
		}
	...
	public static void writeGrayscalePNGFile
		...
		throws IOException
		{
		writeGrayscalePNGFile (theDisplayable, theFile, theScale, 300, 300);
		}

	/**
	 * Write the given displayable object to a PNG file in the form of a
	 * grayscale image with the given scale factor and physical pixel
	 * dimensions. One display unit = <TT>theScale</TT> pixels.
	 *
	 * @param  theDisplayable  Displayable object to write.
	 * @param  theFile         Image file.
	...
	public static void writeGrayscalePNGFile
		...
		OutputStream out = new BufferedOutputStream
			(new FileOutputStream (theFile));
		GrayPngWriter writer = new GrayPngWriter (h, w, out);
		writer.setPixelDimensions (xPpi, yPpi, PixelUnit.PIXELS_PER_INCH);
		final GrayImageQueue queue = writer.getImageQueue();
		new Thread()
			{
			public void run()
				{
				byte[] data = ((DataBufferByte) bi.getRaster().getDataBuffer())
					.getData();
				GrayArray grayArray = new GrayArray (w);
				for (int row = 0; row < h; ++ row)
					{
					for (int col = 0; col < w; ++ col)
						grayArray.gray (col, data[w*row+col]);
					try { queue.put (row, grayArray); }
						catch (InterruptedException exc) {}
					}
				}
			} .start();
	 ...
	 * used. The default physical pixel dimensions (300&times;300 pixels per
	 * inch) are used.
	 *
	 * @param  theDisplayable  Displayable object to write.
	 * @param  theFile         Image file.
	...
	public static void writeColorPNGFile
		...
		throws IOException
		{
		writeColorPNGFile (theDisplayable, theFile, 1.0);
		}
	...
	public static void writeColorPNGFile
		...
		throws IOException
		{
		writeColorPNGFile (theDisplayable, theFile, theScale, 300, 300);
		}

	/**
	 * Write the given displayable object to a PNG file in the form of a 24-bit
	 * color image with the given scale factor and physical pixel dimensions.
	 * One display unit = <TT>theScale</TT> pixels.
	 *
	 * @param  theDisplayable  Displayable object to write.
	 * @param  theFile         Image file.
	 * @param  theScale        Scale factor for converting display units to
	...
	public static void writeColorPNGFile
		...
		OutputStream out = new BufferedOutputStream
			(new FileOutputStream (theFile));
		ColorPngWriter writer = new ColorPngWriter (h, w, out);
		writer.setPixelDimensions (xPpi, yPpi, PixelUnit.PIXELS_PER_INCH);
		final ColorImageQueue queue = writer.getImageQueue();
		new Thread()
			{
			public void run()
				{
				int[] data = ((DataBufferInt) bi.getRaster().getDataBuffer())
					.getData();
				ColorArray colorArray = new ColorArray (w);
				for (int row = 0; row < h; ++ row)
					{
					for (int col = 0; col < w; ++ col)
						colorArray.rgb (col, data[w*row+col] & 0x00FFFFFF);
					try { queue.put (row, colorArray); }
						catch (InterruptedException exc) {}
					}
				}
			} .start();
