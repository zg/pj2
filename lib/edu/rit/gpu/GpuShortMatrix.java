//******************************************************************************
//
// File:    GpuShortMatrix.java
// Package: edu.rit.gpu
// Unit:    Class edu.rit.gpu.GpuShortMatrix
//
...
/**
 * Class GpuShortMatrix provides a short integer matrix GPU variable. This is a
 * two-dimensional array of type <TT>short</TT> stored in the GPU's memory and
 * mirrored in the CPU's memory.
 * <P>
 * Class GpuShortMatrix supports mirroring all of the GPU's data matrix in the
 * CPU's memory, mirroring only a portion of the GPU's data matrix in the CPU's
 * memory, or mirroring none of the GPU's data matrix. Class GpuShortMatrix
 * provides operations for copying all or portions of the data matrix from the
 * CPU to the GPU or from the GPU to the CPU.
 * <P>
 * To use a short integer matrix GPU variable:
 * <OL TYPE=1>
 * <P><LI>
 * Construct an instance of class GpuShortMatrix by calling the {@link
 * Gpu#getShortMatrix(int,int) getShortMatrix()} method on a {@linkplain Gpu
 * Gpu} object, specifying the number of rows and columns in the matrix and
 ...
 * <P><LI>
 * Pass the GpuShortMatrix object as an argument of a GPU kernel function call.
 * In the GPU code, this becomes a pointer (type <TT>short int**</TT> or
 * <TT>unsigned short int**</TT>) to an array of <I>R</I> row pointers, where
 * <I>R</I> is the number of rows in the matrix; each row pointer points to an
 * array of <I>C</I> data elements, where <I>C</I> is the number of columns in
 * the matrix. The GPU code sets the matrix's elements to the desired values.
 * <P><LI>
...
// Exported data members.
	...
	/**
	 * The mirrored portion of the short integer matrix in CPU memory.
	 */
...
// Hidden constructors.
	...
	/**
	 * Construct a new dynamically allocated GPU short integer matrix.
	 *
	...
	GpuShortMatrix
		(Gpu gpu,
		 ...
		 int cpucols)
		{
		super (gpu, rows, cols, cpurows, cpucols, 2L);
		item = new short [cpurows] [cpucols];
		}
