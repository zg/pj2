//******************************************************************************
//
// File:    GpuByteMatrix.java
// Package: edu.rit.gpu
// Unit:    Class edu.rit.gpu.GpuByteMatrix
//
...
/**
 * Class GpuByteMatrix provides a byte matrix GPU variable. This is a
 * two-dimensional array of type <TT>byte</TT> stored in the GPU's memory and
 * mirrored in the CPU's memory.
 * <P>
 * Class GpuByteMatrix supports mirroring all of the GPU's data matrix in the
 * CPU's memory, mirroring only a portion of the GPU's data matrix in the CPU's
 * memory, or mirroring none of the GPU's data matrix. Class GpuByteMatrix
 * provides operations for copying all or portions of the data matrix from the
 * CPU to the GPU or from the GPU to the CPU.
 * <P>
 * To use a byte matrix GPU variable:
 * <OL TYPE=1>
 * <P><LI>
 * Construct an instance of class GpuByteMatrix by calling the {@link
 * Gpu#getByteMatrix(int,int) getByteMatrix()} method on a {@linkplain Gpu Gpu}
 ...
 * <P><LI>
 * Pass the GpuByteMatrix object as an argument of a GPU kernel function call.
 * In the GPU code, this becomes a pointer (type <TT>char**</TT> or <TT>unsigned
 * char**</TT>) to an array of <I>R</I> row pointers, where <I>R</I> is the
...
// Exported data members.
	...
	/**
	 * The mirrored portion of the byte matrix in CPU memory.
	 */
...
// Hidden constructors.
	...
	/**
	 * Construct a new dynamically allocated GPU byte matrix.
	 *
	...
	GpuByteMatrix
		(Gpu gpu,
		 ...
		 int cpucols)
		{
		super (gpu, rows, cols, cpurows, cpucols, 1L);
		item = new byte [cpurows] [cpucols];
		}
