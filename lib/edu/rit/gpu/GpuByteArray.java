//******************************************************************************
//
// File:    GpuByteArray.java
// Package: edu.rit.gpu
// Unit:    Class edu.rit.gpu.GpuByteArray
//
...
/**
 * Class GpuByteArray provides a byte array GPU variable. This is an array of
 * type <TT>byte</TT> stored in the GPU's memory and mirrored in the CPU's
 * memory.
 * <P>
 * Class GpuByteArray supports mirroring all of the GPU's data array in the
 * CPU's memory, mirroring only a portion of the GPU's data array in the CPU's
 * memory, or mirroring none of the GPU's data array. Class GpuByteArray
 * provides operations for copying all or portions of the data array from the
 * CPU to the GPU or from the GPU to the CPU.
 * <P>
 * To use a byte array GPU variable:
 * <OL TYPE=1>
 * <P><LI>
 * Construct an instance of class GpuByteArray by calling either the {@link
 * Gpu#getByteArray(int) getByteArray()} method on a {@linkplain Gpu Gpu} object
 * or the {@link Module#getByteArray(String,int) getByteArray()} method on a
 ...
 * <P><LI>
 * Pass the GpuByteArray object as an argument of a GPU kernel function call. In
 * the GPU code, this becomes a pointer (type <TT>char*</TT> or <TT>unsigned
 * char*</TT>) to an array of <I>N</I> data elements, where <I>N</I> is the
...
// Exported data members.
	...
	/**
	 * The mirrored portion of the byte array in CPU memory.
	 */
...
// Hidden constructors.
	...
	/**
	 * Construct a new dynamically allocated GPU byte array.
	 *
	...
	GpuByteArray
		(Gpu gpu,
		 ...
		 int cpulen)
		{
		super (gpu, len, 1L*len, cpulen);
		item = new byte [cpulen];
		}

	/**
	 * Construct a new statically allocated GPU byte array.
	 *
	 * @param  gpu     Gpu object.
	 * @param  len     Number of elements in GPU memory.
	 * @param  cpulen  Number of elements mirrored in CPU memory.
	...
	GpuByteArray
		(Gpu gpu,
		 ...
		 long dptr)
		{
		super (gpu, len, 1L*len, cpulen, dptr);
		item = new byte [cpulen];
		}
