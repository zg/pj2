//******************************************************************************
//
// File:    GpuFloatMatrix.java
// Package: edu.rit.gpu
// Unit:    Class edu.rit.gpu.GpuFloatMatrix
//
...
/**
 * Class GpuFloatMatrix provides a float matrix GPU variable. This is a
 * two-dimensional array of type <TT>float</TT> stored in the GPU's memory and
 * mirrored in the CPU's memory.
 * <P>
 * Class GpuFloatMatrix supports mirroring all of the GPU's data matrix in the
 * CPU's memory, mirroring only a portion of the GPU's data matrix in the CPU's
 * memory, or mirroring none of the GPU's data matrix. Class GpuFloatMatrix
 * provides operations for copying all or portions of the data matrix from the
 * CPU to the GPU or from the GPU to the CPU.
 * <P>
 * To use a float matrix GPU variable:
 * <OL TYPE=1>
 * <P><LI>
 * Construct an instance of class GpuFloatMatrix by calling the {@link
 * Gpu#getFloatMatrix(int,int) getFloatMatrix()} method on a {@linkplain Gpu
 ...
 * <P><LI>
 * Pass the GpuFloatMatrix object as an argument of a GPU kernel function call.
 * In the GPU code, this becomes a pointer (type <TT>float**</TT>) to an array
 * of <I>R</I> row pointers, where <I>R</I> is the number of rows in the matrix;
 * each row pointer points to an array of <I>C</I> data elements, where <I>C</I>
 * is the number of columns in the matrix. The GPU code sets the matrix's
...
// Exported data members.
	...
	/**
	 * The mirrored portion of the float matrix in CPU memory.
	 */
...
// Hidden constructors.
	...
	/**
	 * Construct a new dynamically allocated GPU float matrix.
	 *
	...
	GpuFloatMatrix
		...
		item = new float [cpurows] [cpucols];
		}
