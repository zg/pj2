//******************************************************************************
//
// File:    GpuShortArray.java
// Package: edu.rit.gpu
// Unit:    Class edu.rit.gpu.GpuShortArray
//
...
/**
 * Class GpuShortArray provides a short integer array GPU variable. This is an
 * array of type <TT>short</TT> stored in the GPU's memory and mirrored in the
 * CPU's memory.
 * <P>
 * Class GpuShortArray supports mirroring all of the GPU's data array in the
 * CPU's memory, mirroring only a portion of the GPU's data array in the CPU's
 * memory, or mirroring none of the GPU's data array. Class GpuShortArray
 * provides operations for copying all or portions of the data array from the
 * CPU to the GPU or from the GPU to the CPU.
 * <P>
 * To use a short integer array GPU variable:
 * <OL TYPE=1>
 * <P><LI>
 * Construct an instance of class GpuShortArray by calling either the {@link
 * Gpu#getShortArray(int) getShortArray()} method on a {@linkplain Gpu Gpu}
 * object or the {@link Module#getShortArray(String,int) getShortArray()} method
 ...
 * <P><LI>
 * Pass the GpuShortArray object as an argument of a GPU kernel function call.
 * In the GPU code, this becomes a pointer (type <TT>short int*</TT> or
 * <TT>unsigned short int*</TT>) to an array of <I>N</I> data elements, where
 * <I>N</I> is the length of the array. The GPU code sets the array's elements
...
// Exported data members.
	...
	/**
	 * The mirrored portion of the short integer array in CPU memory.
	 */
...
// Hidden constructors.
	...
	/**
	 * Construct a new dynamically allocated GPU short integer array.
	 *
	...
	GpuShortArray
		(Gpu gpu,
		 ...
		 int cpulen)
		{
		super (gpu, len, 2L*len, cpulen);
		item = new short [cpulen];
		}

	/**
	 * Construct a new statically allocated GPU short integer array.
	 *
	 * @param  gpu     Gpu object.
	 * @param  len     Number of elements in GPU memory.
	 * @param  cpulen  Number of elements mirrored in CPU memory.
	...
	GpuShortArray
		(Gpu gpu,
		 ...
		 long dptr)
		{
		super (gpu, len, 2L*len, cpulen, dptr);
		item = new short [cpulen];
		}
