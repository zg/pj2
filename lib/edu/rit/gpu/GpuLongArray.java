//******************************************************************************
//
// File:    GpuLongArray.java
// Package: edu.rit.gpu
// Unit:    Class edu.rit.gpu.GpuLongArray
//
...
/**
 * Class GpuLongArray provides a long integer array GPU variable. This is an
 * array of type <TT>long</TT> stored in the GPU's memory and mirrored in the
 * CPU's memory.
 * <P>
 * Class GpuLongArray supports mirroring all of the GPU's data array in the
 * CPU's memory, mirroring only a portion of the GPU's data array in the CPU's
 * memory, or mirroring none of the GPU's data array. Class GpuLongArray
 * provides operations for copying all or portions of the data array from the
 * CPU to the GPU or from the GPU to the CPU.
 * <P>
 * To use a long integer array GPU variable:
 * <OL TYPE=1>
 * <P><LI>
 * Construct an instance of class GpuLongArray by calling either the {@link
 * Gpu#getLongArray(int) getLongArray()} method on a {@linkplain Gpu Gpu} object
 * or the {@link Module#getLongArray(String,int) getLongArray()} method on a
 ...
 * <P><LI>
 * Pass the GpuLongArray object as an argument of a GPU kernel function call. In
 * the GPU code, this becomes a pointer (type <TT>long long int*</TT> or
 * <TT>unsigned long long int*</TT>) to an array of <I>N</I> data elements,
 * where <I>N</I> is the length of the array. The GPU code sets the array's
 * elements to the desired values.
 * <P><LI>
...
// Exported data members.
	...
	/**
	 * The mirrored portion of the long integer array in CPU memory.
	 */
...
// Hidden constructors.
	...
	/**
	 * Construct a new dynamically allocated GPU long integer array.
	 *
	...
	GpuLongArray
		...
		item = new long [cpulen];
		}

	/**
	 * Construct a new statically allocated GPU long integer array.
	 *
	 * @param  gpu     Gpu object.
	 * @param  len     Number of elements in GPU memory.
	 * @param  cpulen  Number of elements mirrored in CPU memory.
